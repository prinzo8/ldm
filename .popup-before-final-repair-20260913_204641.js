(() => {
  ...
  if (modal) {
    ...
    const modalCart = $('[data-modal-cart]', modal);
    const modalMinus = $('[data-modal-cart-minus]', modal);
    const modalPlus = $('[data-modal-cart-add]', modal);
    ...
    const updateModalQuantity = () => {
      ...
      const cart = getCart();
      const entry = cart[currentProductId];

      const quantity = entry
        ? (typeof entry === 'number' ? entry : Number(entry.quantity || 0))
        : 0;

      modalQuantity.textContent = String(quantity);
      if (modalMinus) modalMinus.disabled = quantity <= 0;
    };
    ...
    const changeModalQuantity = (delta) => {
      ...
      let quantity = existing
        ? (typeof existing === 'number' ? existing : Number(existing.quantity || 0))
        : 0;
    ...
    if (modalPlus) {
      modalPlus.addEventListener('click', (event) => {
        event.preventDefault();
    ...
    if (modalMinus) {
      modalMinus.addEventListener('click', (event) => {
        event.preventDefault();
        changeModalQuantity(-1);
      });
    }
    ...
    window.addEventListener('ldm:cart-updated', updateModalQuantity);
  }
  }
})();
