...
function ldm_universe_images_admin_page() {
	...
	<div class="wrap">
		...
		>
			...
			<div style="background:#fff;border:1px solid #dcdcde;padding:25px;">
				...
				<h2>02 — Art de la table</h2>

				<div
					class="ldm-universe-preview"
					data-preview="tableware"
					style="
						height:300px;
						background:#f6f6f6;
						display:flex;
					...
					"
				>
					<?php if ( $tableware_url ) : ?>

						<img
							src="<?php echo esc_url( $tableware_url ); ?>"
							style="width:100%;height:100%;object-fit:cover;"
							alt=""
						>
				...
				<input
					type="hidden"
					name="ldm_universe_image_tableware"
					id="ldm_universe_image_tableware"
					value="<?php echo esc_attr( $tableware_id ); ?>"
				>

				<button
					type="button"
					class="button button-primary ldm-select-universe-image"
					data-target="tableware"
				>
					Choisir une image
				</button>
		...
		<form method="post" style="margin-top:30px;">
			...
			<?php wp_nonce_field( 'ldm_universe_images_save' ); ?>

			<input
				type="hidden"
				name="ldm_universe_image_luminaires"
				id="save_ldm_universe_image_luminaires"
				value="<?php echo esc_attr( $luminaires_id ); ?>"
			>
			...
			<input
				type="hidden"
				name="ldm_universe_image_tableware"
				id="save_ldm_universe_image_tableware"
				value="<?php echo esc_attr( $tableware_id ); ?>"
			>

			<button
				type="submit"
				name="ldm_universe_images_save"
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		document.querySelectorAll('.ldm-select-universe-image').forEach(function (button) {
			...
			button.addEventListener('click', function () {
				...
				const frame = wp.media({
					title: 'Choisir une image pour cet univers',
					button: {
						text: 'Utiliser cette image'
					},
					multiple: false,
				...
				frame.on('select', function () {
					...
					document.getElementById(
						'ldm_universe_image_' + target
					).value = attachment.id;

					document.getElementById(
						'save_ldm_universe_image_' + target
					).value = attachment.id;

					const preview =
						document.querySelector(
							'.ldm-universe-preview[data-preview="' +
							target +
							'"]'
						);
					...
					const removeButton =
						document.querySelector(
							'.ldm-remove-universe-image[data-target="' +
							target +
							'"]'
						);
		...
		document.querySelectorAll('.ldm-remove-universe-image').forEach(function (button) {
