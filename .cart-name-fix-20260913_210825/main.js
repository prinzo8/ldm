(() => {
  ...
  if (modal) {
    ...
    modalAddProduct?.addEventListener('click', () => {
      ...
      const productCard =
        document.querySelector('[data-product-modal][aria-hidden="false"]')
          ? null
          : null;
...
(() => {
  ...
  const render = () => {
    ...
    let meta = {};
    try {
      meta = JSON.parse(localStorage.getItem('ldmCartMeta') || '{}');
    } catch {
      meta = {};
    }
      ...
      items.innerHTML = entries.length
        ? entries.map(([id, qty]) => {
            const product = document.querySelector(
              `[data-product-card][data-product-id="${CSS.escape(id)}"]`
            );
            ...
            const number =
              meta[id]?.number ||
              product?.dataset.productNumber ||
              meta[id]?.number || id;

            return `
              <div class="ldm-cart__item" data-cart-item="${id}">
                <div class="ldm-cart__item-info">
                  <strong class="ldm-cart__item-name">${name}</strong>
                  <span class="ldm-cart__item-number">${number}</span>
                </div>
