...
/**
 * Données minimales des produits pour le panier.
 */
function ldm_cart_products_data() {
    $products = function_exists( 'ldm_get_products' ) ? ldm_get_products() : array();
    ...
    foreach ( $products as $product ) {
        ...
        $data[ $product['id'] ] = array(
            ...
            'priceText'=> $product['price'] ?? 'Prix sur demande',
            'image'    => $product['image'] ?? '',
        );
    }
...
add_action( 'wp_footer', function() {
    ?>
    <script>
        window.ldmCartProducts = <?php echo wp_json_encode( ldm_cart_products_data() ); ?>;
    </script>
    <?php
} );
...
/**
 * Création d'une commande LDM.
 * V1 : enregistre la commande dans WordPress.
 * Le paiement sera branché séparément sur Wave / Orange Money.
 */
function ldm_create_order() {
    if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'ldm_checkout' ) ) {
        wp_send_json_error( array( 'message' => 'Session de commande invalide.' ), 403 );
    }
    ...
    if ( ! $name || ! $phone || ! $address || ! is_array( $items ) || empty( $items ) ) {
        wp_send_json_error( array( 'message' => 'Veuillez compléter les informations nécessaires.' ), 400 );
    }
    ...
    foreach ( $items as $item ) {
        ...
        $qty = max( 1, min( 99, absint( $item['qty'] ?? 1 ) ) );
    ...
    if ( empty( $order_items ) || $total <= 0 ) {
        wp_send_json_error( array( 'message' => 'Aucun produit valide dans cette commande.' ), 400 );
    }
    ...
    $order_id = wp_insert_post(
        array(
            'post_type'   => 'ldm_order',
            'post_status' => 'publish',
            'post_title'  => 'Commande LDM — ' . $name,
        ),
        true
    );

    if ( is_wp_error( $order_id ) ) {
        wp_send_json_error( array( 'message' => 'Impossible de créer la commande.' ), 500 );
    }

    update_post_meta( $order_id, '_ldm_customer_name', $name );
    update_post_meta( $order_id, '_ldm_customer_phone', $phone );
    update_post_meta( $order_id, '_ldm_customer_email', $email );
    update_post_meta( $order_id, '_ldm_customer_address', $address );
    update_post_meta( $order_id, '_ldm_items', $order_items );
    update_post_meta( $order_id, '_ldm_total', $total );
    update_post_meta( $order_id, '_ldm_payment_status', 'pending' );
...
add_action( 'wp_ajax_ldm_create_order', 'ldm_create_order' );
add_action( 'wp_ajax_nopriv_ldm_create_order', 'ldm_create_order' );


/**
 * Type de contenu privé pour les commandes.
 */
function ldm_register_order_post_type() {
    register_post_type(
        'ldm_order',
        array(
            'labels' => array(
...
add_action( 'init', 'ldm_register_order_post_type' );


/**
 * Nonce + URL AJAX accessibles au checkout.
 */
add_action( 'wp_footer', function() {
    ?>
    <script>
        window.ldmCheckout = {
            ...
            nonce: <?php echo wp_json_encode( wp_create_nonce( 'ldm_checkout' ) ); ?>
        };
    </script>
    <?php
} );
...
add_action( 'wp_enqueue_scripts', function() {
    wp_enqueue_script( 'ldm-luminaires-categories', get_template_directory_uri() . '/assets/js/luminaires-categories.js?v=4', array(), filemtime( get_template_directory() . '/assets/js/luminaires-categories.js' ), true );
} );
