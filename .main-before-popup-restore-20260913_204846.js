...
/* ============================================================
   PRODUCT MODAL — CART CONTROLS
   ============================================================ */
(() => {
  const modal = document.querySelector('[data-product-modal]');
  if (!modal) return;

  const minus = modal.querySelector('[data-modal-cart-minus]');
  const plus = modal.querySelector('[data-modal-cart-add]');
  const quantity = modal.querySelector('[data-modal-cart-quantity]');
  ...
  const writeCart = (cart) => {
    ...
    window.dispatchEvent(
      new CustomEvent('ldm:cart-updated', { detail: cart })
    );
  };

  const refresh = () => {
    if (!quantity || !currentProductId) return;

    const cart = readCart();
    const item = cart[currentProductId];

    const qty = item
      ? Number(typeof item === 'object' ? item.quantity : item)
      : 0;

    quantity.textContent = String(Math.max(0, qty));
  ...
  const change = (delta) => {
    ...
    const cart = readCart();
    const existing = cart[currentProductId];

    let qty = existing
      ? Number(typeof existing === 'object' ? existing.quantity : existing)
      : 0;
  ...
  const detectProduct = () => {
    ...
    const card = document.querySelector('[data-product-card].is-active');

    currentProductId =
      modal.dataset.productId ||
      name?.dataset.productId ||
