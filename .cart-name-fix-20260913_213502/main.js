...
/* LDM CART OPEN FALLBACK V1 */
(() => {
  const drawer = document.querySelector('[data-cart-drawer]');
  const triggers = document.querySelectorAll('[data-cart-open]');
  const closeButtons = document.querySelectorAll('[data-cart-close]');

  if (!drawer || !triggers.length) {
    console.warn('LDM panier : drawer ou bouton introuvable');
    return;
  }
...
(() => {
  ...
  const openButton = document.querySelector('[data-cart-open]');
    ...
    console.warn('[LDM CART] éléments du panier introuvables');
    return;
  }
  ...
  openButton.addEventListener('click', openCart, true);
  ...
  console.log('[LDM CART] ouverture du panier OK');
})();


/* ============================================================
   LDM CART DRAWER — OPEN / CLOSE FIX
   ============================================================ */
(() => {
  const drawer = document.querySelector('[data-cart-drawer]');
  const openButton = document.querySelector('[data-cart-open]');

    console.warn('[LDM CART] drawer ou bouton introuvable');
    return;
  }
  ...
  const openCart = (event) => {
    ...
    // Force l'état visuel après le rendu pour permettre l'animation.
    requestAnimationFrame(() => {
      drawer.classList.add('is-visible');
    });
  };
  ...
  // Initialisation : le panneau est fermé.
  drawer.hidden = true;
  drawer.setAttribute('aria-hidden', 'true');
  drawer.classList.remove('is-open', 'is-visible');
  document.body.classList.remove('cart-is-open');

  console.log('[LDM CART] contrôleur chargé');
})();
