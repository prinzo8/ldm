...
(() => {
  ...
  const getProductId = (control) => {
    const card = control.closest('[data-product-card], .product-card');

    return (
      control.dataset.productId ||
      card?.dataset.productId ||
      card?.querySelector('[data-product-id]')?.dataset.productId ||
      card?.querySelector('[data-product-id]')?.value ||
      card?.id ||
      ''
    );
  };
  ...
  const updateControl = (control) => {
    const productId = getProductId(control);
    if (!productId) return;

    const quantity = getQuantity(productId);
    const output = control.querySelector('[data-cart-quantity]');
  ...
  const updateAllControls = () => {
    document.querySelectorAll('[data-cart-product]').forEach(updateControl);

    document.dispatchEvent(new CustomEvent('ldm:cart-render'));
  };
  ...
  document.addEventListener('click', (event) => {
    ...
    const control = (add || minus).closest('[data-cart-product]');
    if (!control) return;
  ...
  document.addEventListener('DOMContentLoaded', updateAllControls);
  window.addEventListener('pageshow', updateAllControls);

  window.ldmCart = {
    get: readCart,
    getQuantity,
    setQuantity,
    update: updateAllControls
  };
})();
...
(() => {
    ...
    const saveCart = (cart) => {
        ...
        window.dispatchEvent(new CustomEvent('ldm:cart-updated', { detail: cart }));
    };

    const getProductId = () => {
        const card = document.querySelector('[data-product-modal].is-open [data-product-id]')
            || document.querySelector('[data-product-modal] [data-product-id]');

        return card?.dataset.productId
            || modal.dataset.productId
            || modal.querySelector('[data-modal-name]')?.textContent.trim()
            || '';
    };

    const getQuantity = () => {
        const id = getProductId();
        const item = getCart().find(entry => String(entry.id) === String(id));
        return item ? Number(item.quantity || 0) : 0;
    };

    const render = () => {
        const output = modal.querySelector('[data-modal-cart-quantity]');
        if (output) output.textContent = getQuantity();
    };
    ...
    const change = (delta) => {
        ...
        const current = index >= 0 ? Number(cart[index].quantity || 0) : 0;
    ...
    modal.addEventListener('click', (event) => {
        if (event.target.closest('[data-modal-cart-add]')) {
            event.preventDefault();
            change(1);
        }

        if (event.target.closest('[data-modal-cart-minus]')) {
            event.preventDefault();
            change(-1);
        }
    });
